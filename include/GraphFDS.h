...
// Class for graphs (fixed # of vertices, directional edges, with edge states)
// Performance is enhanced (but using more memory)
// by having two data sets, one for the in-edges and the other for the out-edges.

class GraphFDS : public GraphF {
   public :
	// Constructors.
   	GraphFDS(): GraphF(0) {};
	GraphFDS(long nv, LinkType lt=0, bool l=false, bool m=false): GraphF(nv,lt,l,m), gra(nv), gra_out(nv) {};
	...
	// read1: only reads (ori, des) from each line (for graphs without link state).
	bool read1(std::istream &ifile) {
		return false;
	};
	// read2: only reads (ori, des, id) from each line (for graphs with link states).
   ...
   protected :
	...
	std::vector<NodeIDLinkIDMMap> gra_out;

	// To get the information on ID.
	// std::map<LinkID,pair<NodeID,NodeID> > links_info; // Not implemented this time to save the memory.
