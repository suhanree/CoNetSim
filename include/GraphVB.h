...
// Class for graphs (varying # of vertices, bidirectional edges, no edge state)
class GraphVB : public GraphV {
   public :
	// Constructor.
	GraphVB(NodeIDSSet &vs, LinkType lt=0, bool l=false, bool m=false): GraphV(vs,lt,l,m), gra() {};
	...
	// (Returns false if it doesn't exist or unsuccessful).
	bool remove_edge(NodeID ori, NodeID des); // For all graphs (remove all edges between two nodes).
	bool remove_edge_ID(NodeID ori, NodeID des, LinkID e) { //Remove the specified edge, e, between two nodes. 
		return false;			 // For graphs with edge states (classes that ends with S).
	};
