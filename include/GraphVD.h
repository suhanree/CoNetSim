...
// Class for graphs (varying # of vertices, directional edges, no edge state)
// By having two data sets, one for the in-edges and the other for the out-edges.
class GraphVD : public GraphV {
   public :
	// Constructors.
	GraphVD(NodeIDSSet &vs, LinkType lt=0, bool l=false, bool m=false): GraphV(vs,lt,l,m), gra(), gra_out() {};
	...
	bool remove_edge_ID(NodeID ori, NodeID des, LinkID e) { //Remove the specified edge (not allowed)
		return false;		
	};
   ...
   private :
	...
	std::map<NodeID,NodeIDMSet> gra_out;
