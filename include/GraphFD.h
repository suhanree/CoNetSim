...
// Class for graphs (fixed # of vertices, directional edges, no edge state)
// Enhanced (but using more memory) version
// 	by having two data sets, one for the in-edges and the other for the out-edges.
class GraphFD : public GraphF {
   public :
	// Constructors.
   	GraphFD(): GraphF(0) {};
	GraphFD(long nv, LinkType lt=0, bool l=false, bool m=false): GraphF(nv,lt,l,m), gra(nv), gra_out(nv) {};
