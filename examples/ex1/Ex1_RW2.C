...
// Random-walking nodes in a periodic 2D lattice with opinions (0<op<1).
...
void ParameterSet::error_check() const {
	...
	if (double(nnodes)*(nnodes-1)< 2.1474836475e+9 && static_cast<double>(num_edges) > static_cast<double>(nnodes)* \
		(static_cast<double>(nnodes)-1.0)) {
...
void InputParamRW2::read_all_parameters() {
	...
	if (!get("op_threshold",param.op_threshold,cerr)) {
		cerr << "# No parameter given: op_threshold";
		cerr << ". Default value, " << param.op_threshold;
		cerr << ",  will be used.\n";
	};
	if (!get("opinion_topology",param.opinion_topology,cerr)) {
		cerr << "# No parameter given: opinion_topology";
		cerr << ". Default value 2 (circular) will be used.\n";
	};
	if (!param.if_nodes_in && !get("op_init_method",param.op_init_method,cerr)) {
...
// A function to check the convergence of the opinion of nodes.
bool check_convergence(RW2NodesBase &nodes, TimeType t, const Graphs &graphs, short method) {
	...
	switch (method) {
	   // Check with the opinion values of all nodes.
	   case 1 :
	   	if (t==0) return false;	// At the start, it cannot compare with the previous step.
		for (NodeID i=0;i<size;i++) {
			double diff=nodes.access_state(i,nt,t).get_op()-nodes.access_state(i,nt,t-1).get_op();
...
// Finding groups of opinions at t based on opinions only 
long find_opiniongroups(RW2NodesBase &nodes, NodeType nt, TimeType t, std::vector<multiset<double> > & opiniongroups, \
	short topology, double thres) {
	if (!nodes.if_type(nt)) throw Bad_Node_Type();
	NodeID nnodes=nodes.get_nnodes();
	double temp=nodes.access_state(0,nt,t).get_op();
	long ngroups=1;
	...
	for (NodeID i=1;i<nnodes;i++) {
		temp=nodes.access_state(i,nt,t).get_op();
		bool ingroup=false;
...
// cout << "create group " << ngroups << '\t' << opinions[ngroups-1] << endl;
		};
	};
...
double find_av_diff(RW2NodesBase &nodes, NodeType nt, TimeType t, short topology, long meshsize, short xory) {
	...
	for (NodeID id=0;id<nnodes;id++) {
		...
		double tem_p=nodes.access_state(id,nt,t).get_op();
		if (xory==1) {
			//if (tem_y!=0) continue;
...
double find_op_width(RW2NodesBase &nodes, NodeType nt, TimeType t, short topology, short xory) {
	if (!nodes.if_type(nt)) throw Bad_Node_Type();
	long size,xsize,ysize;
	xsize=Point2DTorus::get_xsize();
	ysize=Point2DTorus::get_ysize();
	...
	for (int n=0;n<nnodes;n++) {
		...
		double tem_p=nodes.access_state(n,nt,t).get_op();
		if (xory==1) {
...
bool initialize_nodes_states_random(RW2NodesBase &nodes, NodeType nt, TimeType t, short op_init_method, short geometry, PointSet &excluded) {
	...
	for (NodeID id=0; id<nnodes;id++) {
		...
		RW2State st(p.get_x(), p.get_y(), \
			(op_init_method==2 ? double(id)/nnodes+0.5/double(nnodes) : nodes.access_rng(id,nt).fdraw()));
...
int main(int argc, char **argv) {
	...
	// Getting the input parameters for RW2.
	InputParamRW2 in_param(infile,param);
	...
	RW2State::set_op_topology(param.opinion_topology);
	cout << "\tThe topology of the opinion space is ";
	if (param.opinion_topology==1) 
		cout << "linear."<< endl;
	else 
		cout << "periodic."<< endl;
	...
	if (sg_effect) {
		...
		cout << "threshold, " << param.op_threshold << endl;
   ...
   try {
	...
	Graphs_Ex4 graphs1(param.nnodes,time); // will be used when AG is bidirectional
	Graphs_Ex5 graphs2(param.nnodes,time); // will be used when AG is directional
	Graphs *pgraphs;
	if (param.update_method>=3) pgraphs=&graphs1; 	// bidirectional AG
	else pgraphs=&graphs2; 				// directional AG
	...
	RW2Network net(nodes,links,graphs,param);

	// Nodes creation at time=0.
	net.create_nodes(time);
	...
	while (true) {
		...
		net.update_graphs(time);
		net.update_links(time); // Not needed here.
