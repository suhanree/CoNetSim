...
// Random-walking nodes (fixed number) in a periodic 2D lattice with node states (0<phi<1).
...
struct ParameterSet {
	...
	double node_state_threshold;
	short node_state_topology;
	short node_state_init_method;
	...
	ParameterSet() {
		geometry=1;
		node_state_threshold=0.5;
		node_state_topology=2;
		node_state_init_method=1;
		int_range=0;
		neighbor_type_sg=0;
		node_max_speed=1;
...
class InputParamEx2: public InputParam {
   public:
   	// constructor
   	InputParamEx2(const std::string &infile, ParameterSet &p): InputParam(infile,p) {};
	// destructor
	~InputParamEx2() {};
...
//	when method=1, using the node states of all nodes (comparing node states at two subsequent time steps).
...
// A function to find the absolute difference between two node states when node states are linear(1) or circular(2).
double find_phi_diff(double a, double b, short topology);
// A function to find the difference between two node states when node states are linear(1) or circular(2).
double find_phi_diff2(double a, double b, short topology);

// Finding groups of node states based on node states only
long find_nodegroups(RW2NodesBase &nodes, NodeType nt, TimeType t, std::vector<std::multiset<double> > & nodegroups,\
...
double find_phi_width(RW2NodesBase &nodes, NodeType nt, TimeType t, short topology, short xory=1);
